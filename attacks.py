...
sys.path.append("/home/sam/OpenPCDet/src/OpenPCDet")
...
class PointCloudAttack(object):
    ...
    def build_models(self):
        ...
        if self.args.surrogate_model != 'centerpoint' :
            ...
            ckpt_path = '/home/adv_ws/cbgs_voxel0075_centerpoint_nds_6648.pth'
            config_path = '/home/catkin_ws/src/OpenPCDet/tools/cfgs/nuscenes_models/cbgs_voxel0075_res3d_centerpoint.yaml'
            ...
            self.wb_classifier = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=self.demo_dataset)
            # 加载权重文件
            self.wb_classifier.load_params_from_file(filename=ckpt_path, logger=logger, to_cpu=True)
            self.wb_classifier = self.wb_classifier.cuda() # 将网络放到GPU上
            self.wb_classifier.eval() # 开启评估模式

            self.classifier = build_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=self.demo_dataset)
            # 加载权重文件
            self.classifier.load_params_from_file(filename=ckpt_path, logger=logger, to_cpu=True)
            self.classifier = self.classifier.cuda() # 将网络放到GPU上
            self.classifier.eval() # 开启评估模式
